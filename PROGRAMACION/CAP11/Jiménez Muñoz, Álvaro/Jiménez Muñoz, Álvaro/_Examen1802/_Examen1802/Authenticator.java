...
public class Authenticator {
    ...
    public static void main(String[] args){
        ...
        if( (file = obtenerNombreDeArchivo(args)).equals("")){
            ...
            //System.out.println("Ocurrió un error loco");
            //System.exit(-1);
        }

        // (0 puntos) TODO si no sabes hacer lo anterior descomenta la siguiente línea
        file = "users.data";//comentar*
        Scanner sc = new Scanner(System.in);
        try {
            Usuarios usuarios = new Usuarios(file);
            if(usuarios.getFile()== null || usuarios.getFile().isEmpty()){
                sc.close();
                System.out.println("No se ha podido crear el archivo de usuarios");
                return;
            }
            boolean salir = false;
